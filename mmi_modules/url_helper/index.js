    ...
    const promise = new Promise((resolve, reject) => {
        ...
        try{
            if(this._request_source === 'cloudscraper'){
                $cloudscraper.get(_uri, {headers: headers, timeout: 120000}, function(error, response, body){
                ...
                $request.get(_uri, {headers: headers, timeout: 120000}, function(error, response, body){
                    if(error) reject(error);
                    else if(!body && !error) reject('Something is wrong with the website.');
                    else resolve(body);
                })
